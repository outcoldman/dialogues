  ...
  */
  this.load = function() {
    return $.getJSON(this._options.server, { id: this._options.id, host: this._options.host }, function(data) {
      ...
      if (typeof this.socketsClient === 'undefined' && this._options.load.sockets) {
        this.socketsClient = SocketsClient(this._options.server);

        if (this.socketsClient) {
          this.socketsClient.subscribe({
