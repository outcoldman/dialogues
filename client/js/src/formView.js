...
function($, RestClient, storage, cookies, EventsAggregator) { 'use strict';

var FormView = function(options, $parent) {
  var that = this;

  var host = options.host;
  var id = options.id;
  var formRender = options.formRender;
  var bodyFormatter = options.bodyFormatter;
  ...
  var eventsAggregator = EventsAggregator.get(host, id);
  ...
  var _onSubmit = function() {
    ...
    restClient.post(comment, function(err, result) {
      if (err) {
        ...
        _resetForm();
        eventsAggregator.raise('submitted', result);
      }
    });
  };
