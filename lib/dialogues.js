...
var Dialogues = function(opts) {
  ...
  */
  ...
  var _notifySocketListeners = function(dialogue, updatedCommentaries) {
    var id = dialogue.host + '_' + dialogue.id;
    var listeners = this._socketListeners[id];
    if (listeners) {
  ...
  var _onSubscribeSocket = function(dialogue, socket) {
    var id = dialogue.host + '_' + dialogue.id;
    if (!this._socketListeners[id]) {
      this._socketListeners[id] = [];
    }
    // In case of if the same socket is trying to connect twice.
    if (this._socketListeners[id].indexOf(socket) < 0) {
      this._socketListeners[id].push(socket);
    }
  }.bind(this);
