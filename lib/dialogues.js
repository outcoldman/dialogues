...
var updateExpirationTimeout = 10000;
...
var handlePost = function(req, res) {
  ...
  req.on('end', function() {
    ...
    storage.addComment(
      ...
      data.comment,
      function(comment) {
        // Generate token for update
        crypto.randomBytes(48, function(ex, buf) {
          var updateToken = { 
            expired: toUTCDate(new Date((new Date()).getTime() + updateExpirationTimeout)),
            token = buf.toString('hex')
          };

          updateTokens[comment.id] = updateToken;

          setTimeout(function() {
            delete updateTokens[comment.id];
          }, updateExpirationTimeout)
